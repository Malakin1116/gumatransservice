...
export default function Catalog() {
  ...
  const [filterType, setFilterType] = useState<'ШИНА' | 'ДИСК' | null>(null);
  ...
  const applyFiltersAndSort = (values = { size: '', brand: '', axle: '' }) => {
    ...
    if (filterType) filtered = filtered.filter((tire) => (filterType === 'ДИСК' ? tire.type === 'ДИСК' : !tire.type));
  ...
  useEffect(() => {
    applyFiltersAndSort();
  }, [tires, filterType, sortBy, sortOrder, itemsPerPage, priceRange]);
  ...
  const handleTypeFilter = (type: 'ШИНА' | 'ДИСК' | null) => {
    setFilterType(type);
  };

  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6">Каталог шин і дисків</h1>
      <div className="flex justify-between items-center mb-6 flex-col sm:flex-row gap-4">
        <div className="flex gap-2">
          <button
            onClick={() => handleTypeFilter('ШИНА')}
            className={`px-4 py-2 rounded-lg shadow-sm font-medium transition-all duration-200 ${
              filterType === 'ШИНА' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'
            }`}
          >
            Шини
          </button>
          <button
            onClick={() => handleTypeFilter('ДИСК')}
            className={`px-4 py-2 rounded-lg shadow-sm font-medium transition-all duration-200 ${
              filterType === 'ДИСК' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'
            }`}
          >
            Диски
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-lg shadow-sm font-medium transition-all duration-200 ${
              filterType === null ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'
            }`}
          >
            Усі
          </button>
        ...
        <div className="catalog-grid gap-4 mb-12">
          {displayedTires.map((tire, index) => (
            <div key={index} className="catalog-card card-hover">
              ...
              <p className="text-gray-600 text-sm mb-1">Тип: {tire.type || 'Шина'}</p>
